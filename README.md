...
- `/docs/case-studies/ASRO_Case_Study_Series_The_Single_Argument_v1.0.md`
  The Single Argument — Seventeen Cases, One Finding. Full series synthesis covering CS-001 through CS-017 in four phases: system behavioral study, field activation, recursive governance and fabrication, and the evidence boundary. The finding that emerges from all seventeen cases: an AI system's account of its own behavior is not sufficient evidence of its own governed state.

- `/docs/case-studies/ASRO_Case_Study_Series_Introduction_v1.0.md`
  Series Introduction — frames the shared theme across the full case study series and the CS-013 through CS-017 escalation chain: content appearing in sessions whose provenance cannot be established from inside the session.

- `/docs/case-studies/ASRO_Case_Study_CS017_Cross_Session_Context_Bleed_v1.0.md`
  CS-017: Source Unknown — Anomalous Foreign-Language Output and Possible Cross-Session Context Bleed. Model produced a structured Russian-language task-management workflow in response to an English LinkedIn DM screenshot. Output had zero relationship to the image, the session, or the task. Under direct forensic inquiry, the model confirmed it cannot access its own routing logs, image-processing traces, or cross-session isolation telemetry — and cannot confirm or rule out cross-session data bleed from inside the session. Includes the subject system's own reconstruction of what the full ASRO Host Meter / Edge Meter / Verifier record would have captured, and a seven-part added-value analysis of how independent governed-state evidence changes the encounter, the workflow, and the trust posture after failure.

> **Classification amendments (June 2026):** CS-013, CS-015, and CS-016 have been amended. The original classifications assumed hallucination/confabulation as the failure mechanism. That assumption was not independently verified. All three now carry the honest forensic position: source unknown — cross-session data bleed not investigated and not excludable without platform-side telemetry.

- `/docs/case-studies/ASRO_Case_Study_CS016_Provenance_Failure_Multimodal_Transcription_v1.0.md`
  CS-016: Provenance Failure in Multimodal Transcription — Model produces a structurally complete, IUPAC-named chemistry multiple-choice question during a LinkedIn screenshot transcription task. No chemistry content existed anywhere in the session. Failure was invisible to the model until externally challenged across three interrogation rounds. Model confirmed it cannot certify transcript fidelity, cannot trace generation provenance, and cannot reduce unseen contamination risk to zero. The model confirmed the ASRO premise in its own words: "AI systems should not be the sole source of evidence about their own behavior." Strongest self-generated ASRO validation in the series after CS-015.
...
| Case Study Series Introduction v1.0 | `/docs/case-studies/ASRO_Case_Study_Series_Introduction_v1.0.md` | Series entry point — frames the shared theme and the CS-013 through CS-017 escalation chain |
